...
### Added
- **Event Hooks Exposure** — `/hooks` command to list active hooks, register new hooks at runtime. Codex users (+398) have been waiting 6+ months for this feature.
- **VS Code Extension** — synced to v2.6.0, ready for marketplace publishing.
- **pyproject.toml** — version catch-up fix (was 2.3.0 behind actual 2.5.0).

## [2.5.0] - 2026-06-29

### Added
- **Model Auto-Discovery** — `/discover` command probes any OpenAI-compatible endpoint and auto-discovers available models. Works with Ollama, vLLM, LM Studio, LocalAI, and any OpenAI-compatible API (opencode +167).
...
### Added
- **External Editor Integration** — respects $VISUAL/$EDITOR, `/editor` command, vim/nvim/code/emacs/nano support with line number jump (gemini-cli +62).
...
### Added
- **Sensitive File Guard** — prevents reading/editing credentials, keys, .env files, and gitignored paths (top codex request +447).
- **`/btw` command** — inject context mid-task without triggering a full agent turn (opencode +318).
- **`/expand` command** — view full content of truncated messages (opencode +255).

### Tests
- 17 new unit tests for sensitive file guard, /btw, and /expand handlers.
